...
`;
  ...
  // Using gemini-2.5-flash as it supports search grounding and exists in the model list
...
【重要ルール（超初心者向け）】
- **プログラミング用語（Python、パイソン、VBA、GAS、マクロ、コード、開発など）や難解なIT専門用語（LLM、ファインチューニング、トークン、パラメータ、API、RAGなど）は【絶対に】使用しないでください。**
- AIのニュースには難しい言葉（例：マルチモーダル、音声エージェント、ウェブグラウンディングなど）が多く出てきますが、これらをそのまま使わず、必ず「写真や声も同時に扱える機能」「声で自然に会話できるアシスタント」「インターネットの最新情報から調べて回答する機能」などのように、超初心者が直感的に理解できるやさしい日本語にかみ砕いて表現してください。
- パソコンやスマホの基本的な操作（文字入力やコピペ、クリック）さえできれば、誰でも今すぐ試せるような実用的なアップデートやニュースを優先して選んでください。プログラミングが必要な機能や、エンジニア向けの機能は紹介しないでください。
